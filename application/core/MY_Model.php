...
class MY_Model extends CI_Model {
	...
	/**
	* $field truyền vào mảng các cột cần lấy
	* $table là table gốc
	* $table_join truyen vào mảng 2 chiều gồm :
	* - $key chiều thứ nhất của mảng la tên table cần join
	* - $value chiều thứ nhất của mảng là một mảng gồm : 
	* 	 	+ $col là cột cần so sánh ( ON )
	*		+ $join kiểu join VD: inner, lert, right, ...
	*/
	function join_table($field = array(), $table_join = array(), $table = ''){
		...
		$this->db->from($table);
		/**
		* Gán giá trị cho bảng cần join 
		* $col cột cần join
		* $join kiểu join VD: inner, lert, right, ... mạc định là inner join
		*/
		foreach ($table_join as $key => $value) {
			if(count($value) != 2 || !isset($value['col']) || !isset($value['join_type'])) {
			return FALSE;
		}
			$join["$key"] = array(
				'col' => $value['col'],
		...
		foreach ($join as $key => $value) {
			$col = $value['col'];
			$on = "$key.$col = $table.$col";
			$this->db->join($key, $on , $value['join']);
			$table = $key;
		}
		$query = $this->db->get();
